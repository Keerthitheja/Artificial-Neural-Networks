...
figure
for i=1:netNum
    plot(tr{i}.perf,'DisplayName',nets{i}.trainFcn,'LineWidth',3);
    hold on
end
set(gca, 'YScale', 'log', 'FontSize',20)
xlabel('Number of Epochs');
ylabel('Mean Square Error ( Log Scale) ');
legend('traingd','traingda','traingdx','trainscg','trainbfg','trainlm','trainbr','location','Best');
hold off;
...
hold on;
for i=1:netNum
    plot(time(i,:),perf(i,:),'DisplayName',nets{i}.trainFcn,'LineWidth',3);
    hold on;
end
set(gca, 'YScale', 'log', 'FontSize',20)
xlabel('Time (secs)');
